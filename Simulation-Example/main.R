...
# Get simulated data
data <- datagen(y="con",N = 2000, k = 20, random_d = "linear", theta = "con_non", var = 1)
k <- ncol(data)-3
...
B <- 500 # Number of Bootstrap repetitions
...
tau.forest <- causal_forest(df_aux[,covariates], df_aux$y, df_aux$d,tune.parameters = "all",num.trees = 4000,
...
ggplot(mm[mm$variable=="pred",], aes(y=value,x=Method))+
  geom_boxplot() +
  theme_cowplot()+
  geom_hline(yintercept=mean(df_main$theta),color="blue")



# Here we have to build a plot function but do not reorder the values since we want to compare them with the true effect.
prep_noorder <- function(cate){
  cate1 <- melt(cate)
  cate1$ID <- rep(1:nrow(cate),times=3)
  return(cate1)
}

mm_DR <- prep_noorder(cate_DR)
mm_R <- prep_noorder(cate_R)
mm_T <- prep_noorder(cate_T)
mm_X <- prep_noorder(cate_X)
mm_CBART <- prep_noorder(cate_CBART)
mm_CF <- prep_noorder(cate_CForest)

mm <- rbind(mm_DR,mm_R,mm_T,mm_X,mm_CBART,mm_CF)
mm$Method <- rep(c("DR-learner","R-learner","T-learner","X-learner","Causal-BART","Causal-Forest"),each=nrow(mm_DR))


mm_s <- mm[mm$variable=="pred",]
mm_s$true <- rep(df_main$theta)

ggplot(mm_s,aes(y=true,x=value,group=Method))+
  geom_point()+
  #scale_alpha_manual(values=c(1.0,0.1,0.1),labels = c("CATE","CI lower 5%", "CI upper 95%")) +
  theme_cowplot() +
  facet_wrap( ~ Method, scales="free", nrow=4) + # Facet wrap with common scales 
  labs(x = "Estimated CATE ", y = "True CATE") +
...
mean((cate_DR$pred-df_main$theta)^2)
mean((cate_R$pred-df_main$theta)^2)
mean((cate_T$pred-df_main$theta)^2)
mean((cate_X$pred-df_main$theta)^2)
mean((cate_CBART$pred-df_main$theta)^2)
mean((cate_CForest$pred-df_main$theta)^2)
